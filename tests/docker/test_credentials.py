...
def test_no_credentials(coresys: CoreSys):
    ...
    assert not docker._get_credentials("ghcr.io/openpeerpower")
    assert not docker._get_credentials("ghcr.io/openpeerpower/amd64-supervisor")
...
def test_matching_credentials(coresys: CoreSys):
    ...
    credentials = docker._get_credentials("ghcr.io/openpeerpower/amd64-supervisor")
    assert credentials["registry"] == "ghcr.io"

    credentials = docker._get_credentials("openpeerpower/amd64-supervisor")
